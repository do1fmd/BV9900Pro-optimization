...
Also most messenger apps won't work properly. This could be fixed by setting ro.freeme_freemanager and ro.hct_autostart_manager to 0 in build.prop.

Instead of editing build.prop directly, I would suggest to use the Magisk plug-in "MagiskHide Props Config".
In older versions of Magisk, you could go to the module section and install "MagiskHide Props Config" from the Magisk repository.
In newer versions, the online repository is no longer an option. You have to [download a release from GitHub](https://github.com/Magisk-Modules-Repo/MagiskHidePropsConf/releases) and place it in a folder of your phone.
Then install the module from this folder in the module section of Magisk.
...
On the phone are two possible task killers installed, which kill background apps regularly. If an app got killed, it is not possible to receive any messages any more or make any notifications. Even SMS do not get delivered any more, if the SMS app gets killed.
### Duraspeed
The first task killer is called Duraspeed. It is no longer installed, though, if your phone came with Android 10.
This app does not have an launcher icon and is killing other apps in the background.
If you do not get notifications Duraspeed may has killed the application. Sometimes duraspeed kills the SMS app und no SMS will be delivered any more.
